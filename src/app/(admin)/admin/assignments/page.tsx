...
export default function AdminAssignmentsPage() {
  ...
  // Query all classes / offerings for admin
  const { data: classes, isLoading: classesLoading } = useQuery({
    queryKey: ["admin-assignment-classes"],
    queryFn: async () => {
      try {
    ...
    queryFn: async () => {
      ...
      try {
        const res = await axiosClient.get(
          `/courses/classes/${selectedClassId}/assignments`,
        );
        return Array.isArray(res) ? res : [];
      } catch {
        return [];
  ...
  const gradeMutation = useMutation({
    mutationFn: async ({
      ...
      return axiosClient.put(`/courses/submissions/${submissionId}/grade`, {
        grade,
        feedback,
      });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin-assignment-detail"] });
      queryClient.invalidateQueries({
        queryKey: ["admin-assignments", selectedClassId],
      });
    },
  ...
  const { data: assignmentDetail, isLoading: detailLoading } = useQuery({
    queryKey: ["admin-assignment-detail", selectedAssignment?.id],
    queryFn: async () => {
      if (!selectedAssignment) return null;
      try {
        const res = await axiosClient.get(
          `/courses/assignments/${selectedAssignment.id}`,
        );
        return res as any;
      } catch {
        return null;
  ...
  if (classesLoading)
    return <div className="p-12 text-center text-slate-500">Đang tải...</div>;
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            Giao bài tập tự luận hoặc trắc nghiệm, chấm điểm tự động và thủ công
          </p>
        </div>
      </div>
          ...
          <div className="flex flex-col gap-2">
            {classes?.map((cls: any) => (
              <button
                ...
                className={`text-left px-4 py-3 rounded-lg transition-colors flex flex-col ${
              ...
              >
                ...
                <span className="text-xs flex items-center gap-1 opacity-80">
                  <Users size={12} /> {cls.studentCount || 0} học viên
                </span>
              </button>
            ))}
            {classes?.length === 0 && (
        ...
        <div className="col-span-1 md:col-span-3">
          {!selectedClassId ? (
            <div className="h-full min-h-[300px] bg-slate-50 rounded-xl border border-dashed border-slate-300 flex flex-col items-center justify-center text-slate-500">
              ...
              <p>Chọn một gói học bên trái để xem bài tập</p>
            </div>
          ) : (
            <div className="bg-white rounded-xl shadow-xs border border-slate-200 overflow-hidden">
              <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                ...
                <button
                  ...
                  className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-medium flex items-center gap-2 text-sm transition-colors cursor-pointer"
                >
              ...
              <div className="p-0">
                {assignmentsLoading ? (
                            ...
                            {paginated.map((asgn: any) => (
                              ...
                              >
                                <div>
                                  ...
                                  <p className="text-sm text-slate-500 mb-2 truncate max-w-lg">
                                    {asgn.description || "Không có mô tả"}
                                  </p>
                                  <div className="flex gap-4 text-xs font-medium">
                                    <span className="text-blue-600 bg-blue-50 px-2 py-1 rounded">
                                      Đã nộp: {asgn.submissions?.length || 0}
                                    </span>
                                    {asgn.dueDate && (
                                      <span className="text-orange-600 bg-orange-50 px-2 py-1 rounded">
                                        Hạn:{" "}
                                        {dayjs(asgn.dueDate).format(
                                          "DD/MM/YYYY HH:mm",
                                        )}
                                      </span>
                                    )}
                                ...
                                <button
                                  ...
                                  className="px-4 py-2 border border-blue-200 hover:bg-blue-50 text-blue-600 font-medium rounded-lg text-sm transition-colors flex items-center justify-center gap-2 cursor-pointer"
                                >
                                  Chấm điểm / Xem bài
                                </button>
                              </div>
                  ...
                  <div className="p-12 text-center text-slate-500">
                    Chưa có bài tập nào được giao cho gói học này.
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl flex flex-col">
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full border border-slate-300 rounded-lg px-4 py-2 outline-none focus:border-blue-500 text-sm"
                  placeholder="Vd: Bài tập tự luận số 1"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full border border-slate-300 rounded-lg px-4 py-2 outline-none focus:border-blue-500 min-h-[100px] text-sm"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full border border-slate-300 rounded-lg px-4 py-2 outline-none focus:border-blue-500 bg-white text-sm"
                  >
                    <option value="ESSAY">Tự luận (Text/Link/File)</option>
                    <option value="QUIZ">
                      Trắc nghiệm trực tuyến (Tự động chấm)
                    </option>
                  </select>
                </div>
                <div>
                ...
                <div className="mt-6 border-t border-slate-200 pt-6">
                  ...
                  <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
                    <input
                      ...
                      className="w-full border border-slate-300 rounded-lg px-4 py-2 outline-none focus:border-blue-500 mb-3 bg-white text-sm"
                    />
                      ...
                      {quizQuestion.options.map((opt, idx) => (
                        <div key={idx} className="flex items-center gap-2">
                          ...
                          <input
                            ...
                            className="flex-1 border border-slate-300 rounded-lg px-3 py-1.5 outline-none focus:border-blue-500 text-sm bg-white"
                          />
                        </div>
                      ))}
                    </div>
                    <button
                      ...
                      disabled={
                        ...
                        quizQuestion.options.some((o) => !o)
                      }
                      className="bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white px-4 py-2 rounded-lg font-medium text-sm transition-colors cursor-pointer"
                    >
                      + Thêm câu hỏi này
                    </button>
                  </div>
            ...
            <div className="p-6 border-t border-slate-100 flex justify-end gap-3 bg-slate-50 mt-auto sticky bottom-0">
              ...
              <button
                ...
                className="px-6 py-2 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white rounded-lg font-medium transition-colors cursor-pointer text-sm"
              >
          ...
          <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl flex flex-col">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center sticky top-0 bg-white z-10">
              <div>
                ...
                >
                  {selectedAssignment?.type === "QUIZ"
                    ? "Bài Trắc nghiệm (Tự động chấm)"
                    : "Bài Tự luận (Chấm thủ công)"}
                </span>
              </div>
              <button
                ...
                <div className="space-y-4">
                  {(assignmentDetail?.submissions || []).map((sub: any) => {
                    const isLate =
                      ...
                      sub.submittedAt &&
                      dayjs(sub.submittedAt).isAfter(
                        dayjs(selectedAssignment.dueDate),
                      );
                    ...
                    return (
                      ...
                      >
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <h4 className="font-bold text-slate-800">
                                {sub.user?.profile?.fullName || sub.user?.email}
                              </h4>
                              <div className="flex items-center gap-2 mt-0.5">
                                <p className="text-xs text-slate-500">
                                  Nộp lúc:{" "}
                                  {dayjs(sub.submittedAt).format(
                                    "DD/MM/YYYY HH:mm",
                                  )}
                                </p>
                                {isLate ? (
                          ...
                          {selectedAssignment?.type === "QUIZ" ? (
                            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 text-sm">
                              <div className="flex items-center gap-2 text-purple-700 font-bold mb-1">
                                <span>
                                  ⚡ Đã được hệ thống chấm tự động 100%
                                </span>
                              </div>
                              <p className="text-slate-600 text-xs">
                                Học sinh nộp câu trả lời trực tiếp trên hệ thống.
                              </p>
                              <p className="text-emerald-700 font-black text-base mt-2">
                                Điểm đạt: {sub.grade}/10 điểm
                              </p>
                            </div>
                          ) : (
                        ...
                        {/* Manual Grading Form for ESSAY or override */}
                        {selectedAssignment?.type !== "QUIZ" && (
                          <div className="bg-blue-50 p-4 rounded-lg border border-blue-100 mt-4">
                            <h5 className="text-sm font-bold text-blue-800 mb-3">
                              Chấm điểm & Nhận xét (Thủ công)
                            </h5>
                            <form
                              onSubmit={(e) => {
                                e.preventDefault();
                                ...
                                gradeMutation.mutate({
                                  submissionId: sub.id,
                                  grade: Number(
                                    (form.grade as HTMLInputElement).value,
                                  ),
                                  feedback: (
                                    form.feedback as HTMLTextAreaElement
                                  ).value,
                                });
                              }}
                            ...
                            >
                              <div className="flex gap-4 items-center">
                                ...
                                <input
                                  ...
                                  required
                                  className="w-24 border border-slate-300 rounded px-3 py-1.5 outline-none focus:border-blue-500 bg-white text-sm"
                                />
                              </div>
                              <textarea
                                name="feedback"
                                ...
                                className="w-full border border-slate-300 rounded px-3 py-2 outline-none focus:border-blue-500 min-h-[60px] text-sm bg-white"
                              />
                              <button
                                type="submit"
                                disabled={gradeMutation.isPending}
                                className="self-end bg-blue-600 hover:bg-blue-700 text-white px-4 py-1.5 rounded font-medium text-sm transition-colors cursor-pointer"
                              >
                                Lưu điểm
                              </button>
                            </form>

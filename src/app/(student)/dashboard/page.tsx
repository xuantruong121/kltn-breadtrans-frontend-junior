...
import { gamificationService, QuestProgress } from "@/lib/api/services/gamification.service";
...
const SKILLS = [
  ...
  { title: "Luyện nói", description: "Ghi âm, luyện phản xạ và nhận góp ý phát âm từ AI.", href: "/practice/speaking", icon: Mic, tone: "violet" },
  ...
  { title: "Luyện viết", description: "Viết câu, email và đoạn văn với góp ý từ AI.", href: "/practice/writing", icon: PenTool, tone: "rose" },
] as const;
...
function getQuestHref(quest: QuestProgress["quest"]) {
  if (quest.type === "LEARN_VOCAB") return "/flashcard";
  if (quest.type === "COMPLETE_QUIZ") return "/practice/listening";
  if (quest.type === "DO_SPEAKING") return "/practice/speaking";
  return "/practice";
}
...
export default function DashboardPage() {
  ...
  const { data: quests, isLoading: areQuestsLoading } = useQuery({
  ...
  const completedQuestCount = quests?.filter((item) => item.isCompleted).length ?? 0;
  const totalQuestValue = quests?.reduce((total, item) => total + item.quest.targetValue, 0) ?? 0;
  const currentQuestValue = quests?.reduce((total, item) => total + Math.min(item.currentValue, item.quest.targetValue), 0) ?? 0;
  const dailyProgress = clampPercentage(currentQuestValue, totalQuestValue);
    ...
    <div className="space-y-8 pb-16 font-['Quicksand',sans-serif]" id="dashboard">
      <section className="relative overflow-hidden rounded-3xl border border-amber-200 bg-gradient-to-r from-amber-50 via-white to-orange-50 p-6 shadow-2xs sm:p-8">
        ...
        <div className="relative flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="grid grid-cols-3 divide-x divide-amber-100 rounded-2xl border border-amber-200 bg-white p-3 shadow-2xs sm:min-w-80">
            <div className="px-3 text-center"><span className="block text-xs font-bold text-slate-500">Chuỗi học</span><strong className="mt-1 block text-xl font-black text-slate-900">{isStatsLoading ? "—" : `${stats?.streakCount ?? 0} ngày`}</strong></div>
            <div className="px-3 text-center"><span className="block text-xs font-bold text-slate-500">Điểm tuần</span><strong className="mt-1 block text-xl font-black text-slate-900">{isStatsLoading ? "—" : stats?.weeklyExp ?? 0}</strong></div>
            <Link href="/student/profile?tab=quotas" className="rounded-xl px-3 text-center transition-colors hover:bg-amber-50"><span className="block text-xs font-bold text-slate-500">Bánh mì</span><strong className="mt-1 block text-xl font-black text-amber-700">{isStatsLoading ? "—" : stats?.totalBanhRan ?? 0}</strong></Link>
          </div>
        </div>
      </section>

      <section className="grid gap-6 lg:grid-cols-3" aria-label="Nhiệm vụ và mục tiêu hôm nay">
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-2xs lg:col-span-2 sm:p-7">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div><p className="text-xs font-extrabold uppercase tracking-[0.14em] text-amber-700">Hoạt động hôm nay</p><h2 className="mt-1 text-xl font-black text-slate-900">Nhiệm vụ hằng ngày</h2></div>
            {!areQuestsLoading && <span className="rounded-full bg-amber-50 px-3 py-1 text-xs font-extrabold text-amber-800">{completedQuestCount}/{quests?.length ?? 0} hoàn thành</span>}
          </div>
          <div className="mt-5 space-y-3">
            {areQuestsLoading && <div className="flex items-center gap-3 rounded-2xl border border-slate-100 bg-slate-50 p-4 text-sm font-semibold text-slate-500"><Loader2 className="animate-spin" size={18} /> Đang tải nhiệm vụ hôm nay…</div>}
            {!areQuestsLoading && quests?.map((item) => {
              const progress = clampPercentage(item.currentValue, item.quest.targetValue);
              return <div key={item.id} className={`rounded-2xl border p-4 ${item.isCompleted ? "border-emerald-200 bg-emerald-50/50" : "border-slate-200 bg-white"}`}>
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex min-w-0 items-start gap-3"><span className={`mt-0.5 flex size-6 shrink-0 items-center justify-center rounded-full ${item.isCompleted ? "bg-emerald-500 text-white" : "border-2 border-amber-500 text-amber-700"}`}>{item.isCompleted ? <CheckCircle2 size={15} aria-hidden="true" /> : null}</span><div><p className="font-bold text-slate-900">{item.quest.title}</p><p className="mt-0.5 text-xs text-slate-600">{item.quest.description || `${item.currentValue}/${item.quest.targetValue} tiến độ`}</p></div></div><div className="flex items-center gap-3"><span className="text-xs font-extrabold text-amber-800">+{item.quest.rewardBanh} bánh mì</span>{!item.isCompleted && <Link href={getQuestHref(item.quest)} className="inline-flex min-h-10 items-center justify-center rounded-xl bg-amber-600 px-3 text-xs font-extrabold text-white transition-colors hover:bg-amber-700">Tiếp tục</Link>}</div></div>
                <div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-amber-500 transition-all" style={{ width: `${progress}%` }} /></div>
              </div>;
            })}
            {!areQuestsLoading && !quests?.length && <p className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">Chưa có nhiệm vụ nào được thiết lập cho hôm nay.</p>}
          </div>
        </div>
        <aside className="flex flex-col justify-between rounded-3xl border border-amber-200 bg-amber-50/60 p-6 shadow-2xs sm:p-7">
          <div><div className="flex items-center gap-2 text-amber-800"><Flame size={18} aria-hidden="true" /><span className="text-xs font-extrabold uppercase tracking-[0.14em]">Mục tiêu hôm nay</span></div><h2 className="mt-3 text-xl font-black text-slate-900">Giữ nhịp học của bạn</h2><p className="mt-2 text-sm leading-6 text-slate-600">Tiến độ được tính từ nhiệm vụ hiện có trong ngày.</p><div className="mt-6 rounded-2xl border border-amber-200 bg-white p-4"><div className="flex justify-between text-sm font-bold text-slate-700"><span>Hoàn thành</span><span>{areQuestsLoading ? "—" : `${dailyProgress}%`}</span></div><div className="mt-2 h-3 overflow-hidden rounded-full bg-amber-100"><div className="h-full rounded-full bg-amber-500 transition-all" style={{ width: `${dailyProgress}%` }} /></div></div></div>
          <Link href="/practice" className="mt-6 inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-amber-600 px-4 text-sm font-extrabold text-white transition-colors hover:bg-amber-700">Chọn bài để luyện <ArrowRight size={16} aria-hidden="true" /></Link>
        </aside>
      </section>
